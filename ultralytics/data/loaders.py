...
class LoadTensor:
    ...
    def __init__(self, im0: torch.Tensor, preprocessed: bool = True) -> None:
        """Initialize LoadTensor object for processing torch.Tensor image data.

        Args:
            im0 (torch.Tensor): Input tensor with shape (B, C, H, W).
            preprocessed (bool): Whether the tensor is already preprocessed and at the model
                input size. If False, a raw tensor at original resolution is accepted and letterboxing is deferred
                to the predictor; only normalization to 0.0-1.0 is applied here.
        """
        self.im0 = self._single_check(im0, preprocessed=preprocessed)
    ...
    @staticmethod
    def _single_check(im: torch.Tensor, stride: int = 32, preprocessed: bool = True) -> torch.Tensor:
        ...
        if len(im.shape) != 4:
            ...
            LOGGER.warning(s)
            im = im.unsqueeze(0)
        if preprocessed and (im.shape[2] % stride or im.shape[3] % stride):
            raise ValueError(s)
        if not im.is_floating_point() or im.max() > 1.0 + torch.finfo(im.dtype).eps:  # float32 eps is 1.2e-07
            if preprocessed:
                LOGGER.warning(
