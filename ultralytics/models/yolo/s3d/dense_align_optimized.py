...
class DenseAlignmentOptimized:
    ...
    def _ncc_error(
        ...
        denominator = np.sqrt(
            np.dot(p1_norm.flatten(), p1_norm.flatten()) * np.dot(p2_norm.flatten(), p2_norm.flatten())
        )
    ...
    def _project_box_to_roi(
        ...
        u_coords = (
            fx
            * np.divide(
                corners_x_world[valid_mask], z_valid, out=np.zeros_like(z_valid, dtype=np.float32), where=z_valid != 0
            )
            + cx
        )
        v_coords = (
            fy
            * np.divide(
                corners_y_world[valid_mask], z_valid, out=np.zeros_like(z_valid, dtype=np.float32), where=z_valid != 0
            )
            + cy
        )
    ...
    def refine_depth(
        ...
        for z in z_candidates:
            ...
            if warped_right.shape[:2] != (target_h, target_w):
                warped_right_fixed = cv2.resize(warped_right, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
            else:
